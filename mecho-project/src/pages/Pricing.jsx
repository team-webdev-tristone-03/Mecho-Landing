...
const Pricing = () => {
  ...
  const [viewingMonth, setViewingMonth] = useState(0); // 0=current, 1=next, 2=third month
            ...
            <div className="calendar-content">
              <div className="top-cards">
                ...
                <div className="calendar-header">
                  <h3 className="calendar-subtitle">Select {getPlanLimits()} Wash Dates</h3>
                  <div className="selection-counter">
                    <span className="counter-text">{selectedDates.length}/{getPlanLimits()}</span>
                  </div>
                </div>
                <div className="calendar-grid">
                  <p className="calendar-info">Sundays are disabled for all plans</p>
                  <div className="calendar-navigation">
                    <button 
                      className="nav-btn"
                      onClick={() => setViewingMonth(Math.max(0, viewingMonth - 1))}
                      disabled={viewingMonth === 0}
                    >
                      ←
                    </button>
                    <h4 className="month-title">{getDisplayMonth().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h4>
                    <button 
                      className="nav-btn"
                      onClick={() => setViewingMonth(Math.min(2, viewingMonth + 1))}
                ...
                <div className="time-selection card">
                  <h3 className="calendar-subtitle">Select Time for Each Date</h3>
                  <div className="time-grid">
                    {selectedDates.length > 0 ? (
                      selectedDates.map(dateKey => {
                        const [month, day] = dateKey.split('-');
                        const monthDate = new Date(currentDate.getFullYear(), parseInt(month), 1);
                        const monthName = monthDate.toLocaleDateString('en-US', { month: 'short' });
                        
                        return (
                          <div key={dateKey} className="time-row">
                            <span className="date-label">{monthName} {day}:</span>
                            <select 
                              className="time-select"
                              value={selectedTimes[dateKey] || ''}
                              onChange={(e) => handleTimeSelect(dateKey, e.target.value)}
                            >
